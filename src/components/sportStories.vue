<template>
  <div>
    <app-nav></app-nav>
    <h3 class="text-center"> Daily Sport Stories from across the Globe </h3>
    <hr/>

    <div class="col-sm-4" v-for="story in sportStories">
...
<script>
  ...
  import { getSportStories } from './utils/news-api'
  ...
  export default {
    ...
    methods: {
      getSportStories () {
        getSportStories().then((stories) => {
          this.sportStories = stories.articles
        })
      }
    },
    mounted () {
      this.getSportStories()
    }
  }
</script>
